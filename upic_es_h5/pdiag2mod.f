...
!
      ...
      use pinit2d, only: idrun, indx, indy, ntp, ntd, nta, nte, psolve, &
     &tend, dt, omx, omy, omz, ci, t0, ceng, indian, rlprec, inprec,    &
     &pden2d, modesxd, modesyd, ndrec, fdname, ppot2d, modesxp, modesyp,&
     &nprec, fpname, pvpot2d, modesxa, modesya, narec, faname, pem2d,   &
     &modesxe, modesye, nerec, fename
      implicit none
      private
      ...
      public :: grasp, vdist, displayfv, displays, displayv, displayw
      public :: bfopen
!
...
! mks = 0 = cycle through line styles
         ...
         real :: vmax, vmin
         double precision, dimension(size(fvm,1),size(fvm,2),size(fvm,3)&
     &) :: msg
...
! chrs = short array of characters to label individual line samples
         ...
         nmv2 = 2*nmv
         write (c,91) itime
! send velocity momemts to diagnostic node
         msg = fvm
         call HARTBEAT(msg,size(fvm))
         fvm = msg
...
!
         subroutine ipdisplayw(wt,dtw,nt,irc)
