...
!      integer ks, kc, jc, i, j, k, l, n, nn, koff, noff, nppv
      integer(KIND=8) :: nppvbig, npxy,ks, kc, jc, i, j, k, l, n, nn, ko
     1ff, noff, nppv
      integer imax, iwork
...
c with spatial decomposition
      ...
      external fnx, fny
c local data
      integer ks, npxy, i, l, n, noff, npd, ipp, mdp, ii, jj, nd, id, is
      integer iwork
      real edgelx, edgely, anx, any, bnx, bny, xt, yt, xt0, yt0, x0, y0
      real xn, yn
      ierr = 0
...
c local data
c      integer ks, nppv, npxy, i, j, k, l, joff, imin, npt, iwork
c      integer ks, i, j, k, l, joff, imin, npt, iwork, nppv
!     integer(KIND=8) npxy, nppvbig,ks, i, j, k, l, joff, imin, npt, iwo
!    1rk, nppv
!     real vxt, vyt, at1
!     double precision ranorm
...
c with spatial decomposition
      ...
      integer npx, npy, idimp, npmax, nblok, kstrt, nvp, ierr
      real vxmin,vxmax, vymin,vymax, vdx, vdy
      integer npp, nps
      real part
      dimension part(idimp,npmax,nblok)
      dimension npp(nblok), nps(nblok)
...
c maxwellian velocity distribution
      ...
      i = j + joff
c maxwellian velocity distribution
      vxt = vxmin+randum()*del_vx
      comp_x = randum()
      do while (comp_x .ge. dist_func(vxt))
          vxt = vxmin+ randum()*del_vx
          comp_x = randum()
      end do
      vyt = vymin+randum()*del_vy
      comp_y = randum()
      do while (comp_y .ge. dist_func(vyt))
          vyt= vymin+randum()*del_vy
          comp_y = randum()
      end do
      do 10 l = 1, nblok
...
c-----------------------------------------------------------------------
c different distribution functions here
c 
c-----------------------------------------------------------------------


      real function waterbag(x)
      
      implicit none
      real x
      waterbag=0
      if(x .gt. -1 .and. x .lt. 1) waterbag = 1
      return
      end
      
      real function supergauss4(x)
      
      implicit none
      real x
      
      supergauss4 = exp(-0.5*x**4)
      
      return
      end
      
      real function supergauss5(x)
      
      implicit none
      real x
      
      supergauss5 = exp(-0.5*abs(x)**5)
