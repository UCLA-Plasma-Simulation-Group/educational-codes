...
void MP_Cr8task(void *(*entryPoint) (void *), int *ttype, void (*proc)(),
...
/* Destroy mutex */
   ...
   if (oss)
      fprintf(unit2,"pthread_mutex_destroy error = %d,%s\n",oss,
              strerror(oss));
/* Destroy attribute */
   oss = pthread_attr_destroy(&stackSize);
   if (oss)
      fprintf(unit2,"pthread_attr_destroy error = %d,%s\n",oss,
              strerror(oss));
...
local data                */
   void **param;
   int otype, ostate, nargs, oss, err;
