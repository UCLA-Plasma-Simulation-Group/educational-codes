...
! (none,2d periodic,2d reflecting,mixed reflecting/periodic)
      ...
      real :: temp_we = 0.
			real,dimension(:),allocatable :: U_sumover_x,div_P_sumover_x,jE_sumover_x,S_sumover_x
			integer(KIND=4) :: h5error
			real, dimension(:,:,:,:), pointer :: Eold, ESPoynt, jE, Svec, cutemp
			real,dimension(:,:,:), pointer :: div_ESPoynt
      complex, dimension(:,:,:,:), pointer :: scratcht,Svect
...
! density or potential diagnostics
      if ((ntp > 0) .or. (ntd > 0) .or. (ntfield > 0) .or. (nt_b_field > 0)) then
...
! initialize constants
      ...
      call mkdir_structure(idproc)
			...
			if ((nt_write_div_P_vs_y .ne. 0) .OR. (nt_write_jE_sumover_x .ne. 0)) then
				n_jE_u_poynt = 1
				allocate(div_P_sumover_x(ny),jE_sumover_x(ny),S_sumover_x(ny))
				allocate(Eold(3,nxe,nypmx*kbmin,kblok), jE(3,nxe,nypmx*kbmin,kblok),Svec(3,nxe,nypmx*kbmin,kblok))
				allocate(cutemp(3,nxe,nypmx*kbmin,kblok))
				allocate(ESPoynt(3,nxe,nypmx*kbmin,kblok),div_ESPoynt(nxe,nypmx*kbmin,kblok))
	      allocate(scratcht(3,nyv,kxp,jblok),phit(nyv,kxp,jblok),Svect(3,nyv,kxp,jblok))

				if (idproc==0) then
					fname = './DIAG/div_P_vs_y_file'
					...
					fname = './DIAG/S_sumover_x_file'
					nt_write_S_sumover_x_funit = get_funit(20)
					open(unit=nt_write_S_sumover_x_funit,file=trim(fname),form='unformatted',status='replace')
					write (nt_write_S_sumover_x_funit) ny,nt_write_S_sumover_x,dt,(timerise+timeflat+timefall),tend
				endif
...
! send time step to diagnostic nodes
      ...
      call LOGNAME(label)
  	    ...
  	    call fft(qe,qt,isign,mixup,sct,tfft,indx,indy,kstrt,kyp,inorder)
				call pois(qt,phit,ffc,temp_we,nx,ny,kstrt)
!				isign = 1
!				call fft(phi,sfieldt,isign,mixup,sct,tfft,indx,indy,kstrt,kyp,inorder)
!				call pcguard(phi,kstrt,nvp,kyp,inorder)
!				call cguard(phi,nyp,nx,inorder)
			endif
...
! take transverse part of current
      ...
      !Get ES Poynting vector
			if (n_jE_u_poynt .ne. 0) then
				isign = 1
				!sfield has phi
				call fft(sfield,phit,isign,mixup,sct,tfft,indx,indy,kstrt,kyp,inorder)

				do k = 1, kxp
				do j = 1, nyv
				do i = 1, 3
					ESPoynt(i,j,k,1) = cutemp(i,j,k,1) * sfield(j,k,1)		!P in k space
				enddo
				enddo
				enddo				
				isign = -1
				call fft(ESPoynt,scratcht,isign,mixup,sct,tfft,indx,indy,kstrt,kyp,inorder)
				!put div P into phit
				call ipdivf2(scratcht,phit,nx,ny,kstrt)
				! now store div P at t-dt/2 in div_ESPoynt in real space
				isign = 1
				call fft(div_ESPoynt,phit,isign,mixup,sct,tfft,indx,indy,kstrt,kyp,inorder)
!if (idproc == 0) then
!	print*,itime,div_ESPoynt(1,4,4),phit(4,4,1),cut(1,4,4,1),scratcht(1,4,4,1)
!endif
				...
				!Now store old e field to later get ave E centered at t-dt/2
				Eold = fxyze
			endif
...
! copy data from field to particle partition, and copy to guard cells
      ...
      call bguard(bxyze,nyp,nx,inorder)
			...
			if (n_jE_u_poynt .ne. 0) then
				...
				jE = cutemp * 0.5 * (Eold + fxyze)	!jE now centered at t-dt/2
			  ...
			  ciinv = 1. / ci
				...
				do j = 1, nxe
!				do i = 1, 3
					Svec(1,j,k,1) = ciinv * (fxyze(2,j,k,1) * bxyze(3,j,k,1) - fxyze(3,j,k,1) * bxyze(2,j,k,1))
					Svec(2,j,k,1) = ciinv * (fxyze(3,j,k,1) * bxyze(1,j,k,1) - fxyze(1,j,k,1) * bxyze(3,j,k,1))
					Svec(3,j,k,1) = ciinv * (fxyze(1,j,k,1) * bxyze(2,j,k,1) - fxyze(2,j,k,1) * bxyze(1,j,k,1))
				enddo
				enddo
				
				isign = -1
				call fft(Svec,Svect,isign,mixup,sct,tfft,indx,indy,kstrt,kyp,inorder)
				call ipdivf2(Svect,phit,nx,ny,kstrt)
				! now store div P at t-dt/2 in div_ESPoynt in real space
				isign = 1
				!just use div_ESPoynt as a temp variable
				call fft(div_ESPoynt,phit,isign,mixup,sct,tfft,indx,indy,kstrt,kyp,inorder)

				if ((nt_write_S_sumover_x .ne. 0) .and. (itime*dt > (timerise+timeflat+timefall))) then
					it = itime/nt_write_S_sumover_x
					if (itime == nt_write_S_sumover_x*it) then
						S_sumover_x = 0.
						call get_1D_sumover_x(div_ESPoynt,S_sumover_x,nx,nxe,ny,nypmx,nvp,idproc,inorder)
						if (idproc == 0) then
							write (nt_write_S_sumover_x_funit) S_sumover_x
						endif
					endif
				endif
			endif
...
! field lines diagnostic added by JF
			if (ntlines > 0) then
				...
				if (itime == ntlines*it) then
					call write_Eoft_aty(lines,fxyze,nx,ny,nvp,idproc)
				endif
			endif
...
! call external traveling wave driver
			if (driver_select == 1) then
				...
				call plane_finite_wavelen(fxyze,real(itime)*dt,nx,nxe,nvp,idproc)
			else if (driver_select == 3) then
				call gauss_tran_finite_wavelen(fxyze,real(itime)*dt,nx,nxe,ny,nypmx,nvp,idproc)
			else if (driver_select == 4) then
				call angle_wave(fxyze,real(itime)*dt,nx,nxe,ny,nypmx,nvp,idproc)
			else if (driver_select == 5) then
				call cross_wave(fxyze,real(itime)*dt,nx,nxe,ny,nypmx,nvp,idproc)
			else if (driver_select == 6) then
				call rect_tran_finite_wavelen(fxyze,real(itime)*dt,nx,nxe,ny,nypmx,nvp,idproc)
			else if (driver_select == 7) then
				call gauss_tran_per_wavelen(fxyze,real(itime)*dt,nx,nxe,ny,nypmx,nvp,idproc)
			else if (driver_select == 8) then
				call doub_gauss_tran_finite_wavelen(fxyze,real(itime)*dt,nx,nxe,ny,nypmx,nvp,idproc)
			else if (driver_select == 9) then
				call gauss_tran_per_wavelen_circle_wavefronts(fxyze,real(itime)*dt,nx,nxe,ny,nypmx,nvp,idproc)
			else if (driver_select == 10) then
				call gauss_tran_per_wavelen_pois(fxyze,real(itime)*dt,nx,nxe,ny,nypmx,nvp,idproc)
			else if (driver_select == 11) then
				call rect_tran_per_wavelen(fxyze,real(itime)*dt,nx,nxe,ny,nypmx,nvp,idproc)
			else if (driver_select == 12) then
				call supergauss_tran_per_wavelen(fxyze,real(itime)*dt,nx,nxe,ny,nypmx,nvp,idproc)
			endif
...
! efield diagnostic add by JF
			...
			if (ntfield > 0) then
				...
				if (itime==ntfield*it) then
					...
					sfield = fxyze(3,:,:,:)
					call writef(sfield,nxe,nypmx,itime,itime*dt,EZ,trim(fname),inorder)
				endif
			endif
			endif

! bfield diagnostic add by JF
			if ((dump_start < 0.) .OR. &
				&((real(itime)*dt >= dump_start) .and. (real(itime)*dt <= dump_end))) then
			if (nt_b_field > 0) then
				it = itime/nt_b_field
				if (itime==nt_b_field*it) then
					fname = './DIAG/Bx/'//'pBfieldx.'//cdrun
					sfield = bxyze(1,:,:,:)
					call writef(sfield,nxe,nypmx,itime,itime*dt,BX,trim(fname),inorder)
					fname = './DIAG/By/'//'pBfieldy.'//cdrun
					sfield = bxyze(2,:,:,:)
					call writef(sfield,nxe,nypmx,itime,itime*dt,BY,trim(fname),inorder)
					fname = './DIAG/Bz/'//'pBfieldz.'//cdrun
					sfield = bxyze(3,:,:,:)
					call writef(sfield,nxe,nypmx,itime,itime*dt,BZ,trim(fname),inorder)
				endif
			endif
			endif
			...
			if ((nt_write_U_sumover_x .ne. 0) .and. (itime*dt > (timerise+timeflat+timefall))) then
				...
				if (itime == nt_write_U_sumover_x*it) then
					U_sumover_x = 0.
					call get_2D_sumover_x(0.5*fxyze**2,U_sumover_x,nx,nxe,ny,nypmx,nvp,idproc,inorder)
					if (idproc == 0) then
