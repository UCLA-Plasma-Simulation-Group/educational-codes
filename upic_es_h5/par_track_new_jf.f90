...
public :: setup_tagged_particles_send,write_single_track,get_original_tag
...
!  type( t_species ), intent(in) :: spec
  ...
  do i = 1, track_set%ntracks
  	if (track_set%tracks(i)%part_idx > -1) then
	    call add_single_track_data( track_set%tracks( i ), part, n, t )
  	endif
  enddo
...
subroutine create_track_set_file( track_set, sp_name, ndump, nx, ny,&
   ...
   integer*8, parameter :: p_max_txt_len = 16 
...
!		call mkdir_f_( trim(path), ierr ) !this is made in diag_jf.f
		...
		call h5tset_size_f(typeID, len('tracks',8), ierr)
		...
		! write species name
		call h5tcopy_f(H5T_NATIVE_CHARACTER, typeID, ierr)
		call h5tset_size_f(typeID, len(sp_name,8), ierr)
...
!		 call abort_program()
	  ...
	  read( unit = file_id_tem) track_set%ntracks
	  ...
	  do i = 1, track_set%ntracks
		 read( unit = file_id_tem) tag1, tag2
...
subroutine convert_h5_setup_tracks( track_set, ndump, tags )
   ...
   integer :: i, ierr
		...
		! read tags and setup individual track objects   
		do i = 1, track_set%ntracks
			call setup( track_set%tracks(i), track_set%maxpoints, tags(:,i) )
		enddo
