...
! nmv = number of segments in v for velocity distribution
      ...
      integer :: nterf, nterg, it, modesy2
      integer :: ntasks
      real :: zero = 0.0, we = 0.0, wf = 0.0, wm = 0.0, wef = 0.0
      real :: wke = 0.0, wki = 0.0
      real :: tpush = 0.0, tdpost = 0.0, tdjpost = 0.0
      real :: tpushi = 0.0, tdposti = 0.0, tdjposti = 0.0
      real :: totpush = 0.0, totpushi = 0.0
      real :: tfmove = 0.0, totfmove = 0.0
      real :: trepart = 0.0, totrepart = 0.0
...
! potential diagnostics
      if (ntp > 0) then
         ...
         modesy2 = 2*modesy - 1
         allocate(pott(modesy2,min(modesx,kxp),jblok))
         if (id0==0) then
     ...
     &f,ierr,inorder)
         ...
         if (ntp > 0) then
            if (id0==0) then
               ...
               irc = 0
               fname = 'ppotk2.'//cdrun
               call writef(pott,modesx,modesy2,kxp,11,irc,trim(fname))
            endif
            call plbcast(ktime)
            nprec = ktime(1)
...
! store selected fourier modes
            call gtmodes(sfieldt,pott,nx,ny,modesx,modesy,kstrt)
...
! write diagnostic output
            if (nprec==0) then
               nprec = -1
               fname = 'ppotk2.'//cdrun
               call writef(pott,modesx,modesy2,kxp,11,nprec,trim(fname))
            else
               call writef(pott,modesx,modesy2,kxp,11,nprec)
            endif
         endif
      endif
! transform current to fourier space
...
! begin repartitioning
      if (imbalance >= 0.0) then
      if (pibal.gt.imbalance) then
! initialize timer
         call wtimer(trepart,etime,-1)
...
! record time
         call wtimer(trepart,etime)
         totrepart = totrepart + trepart
         if (id0==0) then
...
!
! accumulate timings
      tpush = tpush + tdpost + tdjpost
      totpush = totpush + tpush
      tpart = tpart + tmove
      tmove = 0.0
      if (movion==1) then
         tpushi = tpushi + tdposti + tdjposti
...
! sort electrons
      if (sortime > 0) then
!        call sortp(part,pt,ip,npp,noff,nyp,npic,tsort,tpush,inorder)
...
! sort ions
      if ((movion==1) .and. (sortimi > 0)) then
         call sortp(parti,pt,ip,nppi,noff,nyp,npic,tsorti,tpushi,inorder&
     &)
...
! accumulate timings for partition manager
      totfmove = totfmove + tfmove
      tfmove = 0.0
      go to 500
 2000 continue
!
...
! send main CPU Time to diagnostic nodes
      ...
      msg(7:8) = totfft; msg(9) = totfmove; msg(10) = totrepart
      ...
      if (id0==0) then
         write (18,*) 'total partition time=', totfmove, 'sec'
         write (18,*) 'total fft time=', totfft, 'sec'
         write (18,*) 'total repartition time=', totrepart, 'sec'
...
! get main CPU Time
            ...
            totfft = msg(7:8); totfmove = msg(9); totrepart = msg(10)
            if (id0==0) then
...
! debug: only periodic writes currently supported
               ...
               if (nprec==0) then
                  nprec = -1
                  fname = 'ppotk2.'//cdrun
                  call writef(pott,modesx,modesy2,kxp,11,nprec,trim(fnam&
     &e))
               else
                  call writef(pott,modesx,modesy2,kxp,11,nprec)
               endif
